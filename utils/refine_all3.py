...
def compose_paramter_vector(A, k, W):
    alpha = np.array([A[0, 0], A[1, 1], A[0, 2], A[1, 2], k[0], k[1]])
    P = alpha
...
def value(P, org_W, X, Y_real):
    ...
    for i in range(M):
        ...
        #计算每幅图的坐标残差
        for j in range(N):
            Y = np.append(Y, get_single_project_coor(
                A, W, np.array([P[4], P[5]]), (X[i])[j]))
...
def jacobian(P, WW, X, Y_real):
    ...
    for i in range(M):
        ...
        for j in range(N):
            res = np.append(res, get_single_project_coor(
                A, W, np.array([P[4], P[5]]), (X[i])[j]))
...
def to_rotation_matrix(zrou):
    ...
    R = np.eye(3, dtype='float') + W * math.sin(theta) + \
        np.dot(W, W) * (1 - math.cos(theta))
